...
int main()
{
    ...
    try
    {
        ...
        // Shader programs
        auto objectShader{CShaderProgram::Build("08-shader.vert", "08-shader.frag")};
        objectShader.Use();
        ...
        {
            float offsetY = y / 10.0f + 0.1f;
            for (int x = -10; x < 10; x += 2)
            {
                float offsetX = x / 10.0f + 0.1f;
                objectShader.SetUniform("offsets[" + std::to_string(index++) + "]", offsetX, offsetY);
            }
        }
