...
int32_t main(int32_t argc, char* argv[])
{
    ...
    try
    {
        textureHandler.AddTexture("..\\Resources\\container.jpg", GL_RGB, GL_CLAMP_TO_EDGE, GL_NEAREST);
        textureHandler.AddTexture("..\\Resources\\awesomeface.png", GL_RGBA, GL_REPEAT, GL_NEAREST);
    }
    catch (const OpenGLException& exc)
    {
    ...
    while (window.IsOpen())
    {
        ...
        if (window.PollKey(GLFW_KEY_UP))
        {
            transparency += 0.001f;
        }

        if (window.PollKey(GLFW_KEY_DOWN))
        {
            transparency -= 0.001f;
        }

        transparency = limitValue(transparency, 0.0f, 1.0f);
        ...
        // Render container
        shaderProgram.Use();
        glUniform1f(shaderProgram.GetUniformLocation("transparency"), transparency);
